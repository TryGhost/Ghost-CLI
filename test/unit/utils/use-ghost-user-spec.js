...
describe('Unit: Utils > ghostUser', function () {
    ...
    describe('shouldUseGhostUser', function () {
        ...
        it('re-throws error if `getGhostUid` throws error', function () {
            const platformStub = sandbox.stub(os, 'platform').returns('linux');
            const execaStub = sandbox.stub(execa, 'shellSync').throws(new Error('Some other error message'));

            try {
                ghostUser.shouldUseGhostUser();
