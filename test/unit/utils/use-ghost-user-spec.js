...
describe('Unit: Utils > useGhostUser', function () {
    it('returns false if platform is not linux', function () {
        ...
        const useGhostUser = proxyquire(modulePath, {
            os: {platform: platformStub}
        });

        const result = useGhostUser();
    ...
    it('re-throws error if one is generated by the `id` command', function () {
        ...
        const execaStub = sinon.stub().throws(new Error('Some other error message'));
        const useGhostUser = proxyquire(modulePath, {
            os: {platform: platformStub},
    ...
    it('returns false if the ghost owner/group is not the owner of the content folder', function () {
        ...
        const fsStub = sinon.stub().returns({uid: 30, gid: 30});
        const useGhostUser = proxyquire(modulePath, {
            os: {platform: platformStub},
            fs: {lstatSync: fsStub},
            execa: {shellSync: execaStub}
        ...
        const result = useGhostUser('/some-dir/content');
        expect(result).to.be.false;
        expect(platformStub.calledOnce).to.be.true;
    ...
    it('returns false if the current user is ghost', function () {
        const platformStub = sinon.stub().returns('linux');
        const execaStub = sinon.stub().returns({stdout: '50'});
        const fsStub = sinon.stub().returns({uid: 50, gid: 50});
        const useGhostUser = proxyquire(modulePath, {
            os: {platform: platformStub},
            fs: {lstatSync: fsStub},
            execa: {shellSync: execaStub}
