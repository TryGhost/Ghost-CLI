...
[![CI](https://github.com/TryGhost/Ghost-CLI/actions/workflows/ci.yml/badge.svg)](https://github.com/TryGhost/Ghost-CLI/actions/workflows/ci.yml)
...
- [Migration bundle format](docs/migration-bundle.md) — `ghost migrate-export` usage, Ghost 6.x fidelity and `--leave-stopped` cutover
...
Releases are cut from the [Release workflow](https://github.com/TryGhost/Ghost-CLI/actions/workflows/release.yml). Run it from `main` and pick a bump type:

- `auto` (default) — minor if there are ✨ commits since the last release, otherwise patch
- `patch` / `minor` / `major` — force a specific bump
- `dry run` — do everything except push, to preview the version

From there it's automatic:

1. The Release workflow runs the tests, then pushes a `chore(release): x.y.z` commit to `main`
2. The [Publish workflow](.github/workflows/publish.yml) publishes that version to npm, creates the `vx.y.z` tag, and creates the GitHub Release
3. Release notes are generated by [`scripts/release-notes.js`](scripts/release-notes.js) from the emoji-prefixed commits since the previous tag — see the [commit message convention](.github/CONTRIBUTING.md#commit-messages)

To preview the notes for a release locally:

```sh
pnpm release-notes v1.30.0 v1.30.1
```
