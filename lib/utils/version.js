...
const utils = {
    ...
    checkCustomVersion(version, versions, activeVersion, opts = {}) {
        ...
        if (!semver.satisfies(parsed, MIN_RELEASE, {includePrerelease: true})) {
            throw new CliError({
    ...
    async versionFromArchive(archivePath, activeVersion = null, opts = {}) {
        ...
        if (!fs.existsSync(archivePath) || !archive.isSupported(archivePath)) {
            throw new SystemError('A supported archive file could not be found.');
        }

        let pkg;

        try {
            pkg = await archive.readPackageJson(archivePath);
        } catch {
            throw new SystemError('Archive file could not be extracted');
        }

        if (!pkg) {
            throw new SystemError('Archive file does not contain a valid package.json.');
        }

        if (pkg.name !== 'ghost') {
            throw new SystemError('Archive file does not contain a Ghost release.');
        }

        if (semver.lt(pkg.version, '1.0.0')) {
            throw new SystemError('Archive file contains pre-1.0 version of Ghost.');
        }
        ...
        if (
            ...
            throw new SystemError('Archive file contains a Ghost version incompatible with the current Node version.');
        }
