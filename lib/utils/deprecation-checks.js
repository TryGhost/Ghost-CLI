...
async function deprecationChecks(ui, system) {
    ...
    const showGhostDeprecation = allInstances
        .some(instance => instance.version && semver.lt(instance.version, '4.0.0'));
    ...
    try {
        let showDatabaseDeprecation = false;
        ...
        for (const instance of allInstances) {
            ...
            if (isProduction && databaseClient === 'sqlite3') { // SQLite is only supported in development
                ...
                if (await mysqlExtension[0].isDeprecated(dbconfig)) {
                    showDatabaseDeprecation = true;
                }
            }
        }
