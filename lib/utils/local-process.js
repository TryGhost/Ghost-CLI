...
Ensure the content folder has correct permissions and try again.`));
        ...
        return new Promise((resolve, reject) => {
            ...
            cp.on('exit', (code) => {
                fs.rmSync(path.join(cwd, PID_FILE), {recursive: true, force: true});
                // If something was written to stderr, log in because it will
                // give the user some clues about what went wrong
                if (errorBuffer) {
            ...
            cp.on('message', (msg) => {
                ...
                if (msg.started) {
                    // Destroying the stream allows the parent process to exit
                    // since there are no more active listeners
                    cp.stderr.destroy();
    ...
    async stop(cwd) {
        ...
        if (!await this._isGhostProcess(pid)) {
            fs.rmSync(pidfile, {recursive: true, force: true});
            return;
        }
    ...
    _checkContentFolder(cwd) {
        ...
        return (stat.mode & OTHERS_READ_WRITE) === OTHERS_READ_WRITE;
    }
