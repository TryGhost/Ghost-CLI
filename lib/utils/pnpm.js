...
/**
 * Runs a pnpm command. Can return a readable stream which allows
...
module.exports = function pnpm(pnpmArgs, options) {
    ...
    // There's no stdout at all if pnpm couldn't be run in the first place
    cp.stdout?.pipe(output, {end: false});
    cp.then(() => output.end(), error => output.destroy(toPnpmError(error)));
