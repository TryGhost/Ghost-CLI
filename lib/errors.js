...
const {GhostError: GhostBaseError} = require('@tryghost/errors/cjs/GhostError');

/**
 * Base CLI Error class. Extends GhostError and augments it
 * with CLI-specific functionality
 *
 * @class CliError
 * @extends GhostBaseError
 */
class CliError extends GhostBaseError {
    constructor(options) {
        const originalError = {};
        ...
        // more accurate stack trace - removes the constructor call
        ...
        this.help = 'You can always refer to https://ghost.org/docs/ghost-cli/ for troubleshooting.';

        if (options.err) {
            const errSource = typeof options.err === 'string' ? {message: options.err} : options.err;

            Object.getOwnPropertyNames(errSource).forEach((property) => {
                ...
                originalError[property] = errSource[property];
            });
        }
