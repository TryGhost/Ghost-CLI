...
class ProcessError extends CliError {
    constructor(options) {
        ...
        options.message = options.message || `Error occurred running command: '${options.command}'`;
        super(options);
    }
    ...
    toString(verbose) {
        ...
        } else if (this.options.exitCode) {
            output += chalk.yellow(`Exit code: ${this.options.exitCode}\n\n`);
        }
