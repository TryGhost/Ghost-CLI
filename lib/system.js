...
class System {
    ...
    getProcessManager(name) {
        ...
        if (!availableProcessManagers[name]) {
            this.ui.log(i18n.t('processManager.errors.doesNotExist', {name: name}), 'yellow');
            return {Class: require('./utils/local-process'), name: 'local'};
        }
        ...
        if (valid !== true) {
            const msg = valid !== false ? i18n.t('processManager.errors.fieldsMissing', {fields: valid.join(', ')})
                : i18n.t('processManager.errors.doesNotInherit');
            this.ui.log(i18n.t('processManager.errors.fallingBack', {name: name, msg: msg}), 'yellow');
            return {Class: require('./utils/local-process'), name: 'local'};
        }

        if (!Klass.willRun()) {
            this.ui.log(i18n.t('processManager.errors.systemIncompatible', {name: name}), 'yellow');
            return {Class: require('./utils/local-process'), name: 'local'};
        }
