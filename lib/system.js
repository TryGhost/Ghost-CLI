...
class System {
    ...
    get globalConfig() {
        if (!this._globalConfig) {
            fs.mkdirSync(this.constructor.globalDir, {recursive: true});
    ...
    addInstance(instance) {
        ...
        const existingInstance = existingNames.find(name => instances[name].cwd === instance.dir);
        ...
        let suffix = 0;
        // Loop through the names, incrementing the suffix by 1 each time until
    ...
    hasInstance(instance) {
        const instances = this.globalConfig.get('instances', {});
        return Object.entries(instances).some(([name, {cwd}]) => cwd === instance.dir && name === instance.name);
    }
    ...
    async hook(hookName, ...args) {
        ...
        for (let extension of this._extensions) {
            ...
            results.push(await extension[hookName].apply(extension, args));
        }
    ...
    _getAvailableProcessManagers() {
        ...
        this._extensions.forEach((ext) => {
            Object.entries(ext.processManagers).forEach(([name, pm]) => {
                if (!fs.existsSync(pm)) {
                    return;
                }
    ...
    writeErrorLog(error) {
        ...
        fs.mkdirSync(path.join(this.constructor.globalDir, 'logs'), {recursive: true});
