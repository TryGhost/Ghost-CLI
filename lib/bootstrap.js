...
const bootstrap = {
    ...
    loadCommand: function loadCommand(commandName, commandPath, yargs, aliases, extensions) {
        ...
        if (!(CommandClass.prototype instanceof Command)) {
            ...
            console.error(i18n.t('command.errors.extensionDoesNotInherit', {name: commandName}));
            return;
        }
    ...
    run: function run(argv, yargs) {
        ...
        if (firstArg === 'help' || firstArg === '--help') {
            ...
            console.error(i18n.t('command.errors.commandNotFound', {name: firstArg}));
            process.exit(1);
        }
        ...
        yargs.wrap(Math.min(100, yargs.terminalWidth()))
            .epilogue(i18n.t('epilogue'))
            .option('D', {
                alias: 'development',
                describe: i18n.t('aliases.global.development'),
                type: 'boolean'
            })
            .option('V', {
                alias: 'verbose',
                describe: i18n.t('aliases.global.verbose'),
                type: 'boolean'
            })
            .options('prompt', {
                describe: i18n.t('aliases.global.prompt'),
                type: 'boolean',
                default: true
            })
            .option('color', {
                describe: i18n.t('aliases.global.color'),
                type: 'boolean',
                default: true
            })
            .version(false)
