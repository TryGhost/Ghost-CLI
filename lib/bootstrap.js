...
const bootstrap = {
    ...
    run: function run(argv, yargs) {
        ...
        yargs.wrap(Math.min(150, yargs.terminalWidth()))
            .epilogue('For more information, see our docs at https://ghost.org/docs/ghost-cli/')
            ...
            .option('e', {
                alias: 'environment',
                describe: 'Environment Ghost should run in. Note that `config.{environment}.json` needs to exist',
                group: 'Global Options:'
            })
            .option('D', {
                alias: 'development',
                describe: 'Alias for --environment development',
                type: 'boolean',
                group: 'Global Options:'
            })
            .option('V', {
