...
async function checkMemory() {
    const memoryInfo = await sysinfo.mem();
    const availableMemory = memoryInfo.available / MB_IN_BYTES;

    if (availableMemory < MIN_MEMORY) {
        throw new SystemError(`You are recommended to have at least ${MIN_MEMORY} MB of memory available for smooth operation. It looks like you have ~${parseInt(availableMemory)} MB available.`);
    }
}
