...
async function ensureBackupFolder(ui, instance) {
    ...
    if (ghostUser.shouldUseGhostUser(contentDir)) {
        ...
        await fsp.mkdir(path.resolve(contentDir, folderName), {recursive: true});
    }
}
    ...
    for (const fileKey in files) {
        ...
        if (fileExists) {
            ...
            if (shouldUseSudo) {
                ...
                await fsp.copyFile(filePath, destinationFilePath);
            }
        }
    }
...
// Formats the current date as `YYYY-MM-DD-HH-mm-ss` in local time
function timestamp() {
    const now = new Date();
    return [
        now.getFullYear(),
...
async function backup(ui, instance) {
...
// Shared with the migration export task, which needs the same filename suffix
