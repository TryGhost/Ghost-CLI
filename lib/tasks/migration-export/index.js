...
module.exports = async function migrationExport(ui, instance, options = {}) {
    ...
    const tasks = [{
        ...
        task: async () => {
            ...
            // The shared API helper tolerates a missing members endpoint for old
            // Ghost releases. A v1 portable bundle must contain both exports.
            for (const file of [contentExportFile, membersExportFile]) {
                if (!(await fsp.stat(path.join(dataDir, file))).size) {
