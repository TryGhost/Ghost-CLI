...
/**
 * Resolves theme entry-point links used by CLI installs and local theme development.
 * Only directory links at this level are supported; nested links remain rejected.
 * Pin targets before output validation so linked themes cannot contain the bundle.
 *
 * @param {string} contentDir
 * @return {Promise<Array<{name: string, source: string}>>}
 */
async function resolveThemes(contentDir) {
    const themesDir = path.join(contentDir, 'themes');
    let stat;
    try {
        stat = await fsp.lstat(themesDir);
    } catch (error) {
    ...
    if (!stat.isDirectory()) {
        throw new SystemError(`Unsupported themes directory: ${themesDir}. Use a regular directory with individual theme links.`);
    }

    const themes = [];
    for (const name of await fsp.readdir(themesDir)) {
        let source = path.join(themesDir, name);
        if ((await fsp.lstat(source)).isSymbolicLink()) {
            try {
                source = await fsp.realpath(source);
            } catch {
                throw new SystemError(`Cannot resolve linked theme: ${path.join(themesDir, name)}. Repair the broken or cyclic link before exporting.`);
            }
            if (!(await fsp.stat(source)).isDirectory()) {
                throw new SystemError(`Linked theme must point to a directory: ${path.join(themesDir, name)}`);
            }
        }
 ...
 * @param {string} contentDir
 * @param {string} destination
 * @param {Array<{name: string, source: string}>} [themes] Preflight-resolved themes
 */
async function copyContent(ui, contentDir, destination, themes) {
    const useSudo = ghostUser.shouldUseGhostUser(contentDir);
    themes = themes || await resolveThemes(contentDir);
    ...
    for (const dir of CONTENT_DIRS) {
        ...
        if (fs.existsSync(source)) {
            if (dir === 'themes') {
                await fsp.mkdir(path.join(destination, dir), {mode: 0o700});
                for (const theme of themes) {
                    await copy(ui, useSudo, theme.source, path.join(destination, dir, theme.name));
                }
            } else {
...
module.exports = async function migrationExport(ui, instance, options = {}) {
    ...
    const themes = await resolveThemes(contentDir);
    const parent = await fsp.realpath(path.dirname(bundleDir));
    const resolvedOutput = path.join(parent, path.basename(bundleDir));
    for (const source of [instance.dir, contentDir, ...themes.map(theme => theme.source)]) {
        ...
        if (overlaps(resolvedOutput, resolvedSource) || overlaps(resolvedSource, resolvedOutput)) {
            throw new SystemError('Output must be outside the source install, content directory and resolved theme targets.');
        }
    }
    ...
    const tasks = [{
        ...
        task: () => copyContent(ui, contentDir, path.join(bundleDir, 'content'), themes)
    }, {
