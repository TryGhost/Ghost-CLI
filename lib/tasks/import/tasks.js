...
const sessionAuthPrompts = [{
    ...
    validate: val => emailRegex.test(`${val}`) || 'You must specify a valid email'
}, {
    type: 'password',
    name: 'password',
    message: 'Enter your Ghost administrator password',
    validate: val => `${val}`.length >= 10 || 'Password must be at least 10 characters long'
}];
 ...
 * @param {boolean} [options.requireMembers] Reject a missing members endpoint
 */
async function exportTask(ui, instance, contentFile, membersFile, {requireMembers = false} = {}) {
    const url = instance.config.get('url');
    const {version} = instance;
    ...
    if (membersFile) {
        await downloadMembersExport(version, url, authData, membersFile, {required: requireMembers});
    }
}
