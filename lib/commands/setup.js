...
class SetupCommand extends Command {
    static configureOptions(commandName, yargs, extensions, onlyOptions) {
        extensions.forEach((extension) => {
            const extensionOptions = extension.config?.options?.setup ?? false;
            if (!extensionOptions) {
                return;
            }

            // Extension options don't override the command's own options
            this.options = {...extensionOptions, ...this.options};
        });
    ...
    tasks(steps) {
        ...
        const extSteps = steps.filter(Boolean).flat().filter(step => step.id && step.task);
        ...
        return [{
            ...
            const task = async (ctx, listrTask) => {
                if (ctx.single || step[internal]) {
                    return step.task(ctx, listrTask);
                }
