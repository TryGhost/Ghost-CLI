...
class SetupCommand extends Command {
    ...
    run(argv) {
        ...
        if (argv.stages && argv.stages.length) {
            ...
            return this.system.hook('setup', this, argv).then(() => {
                ...
                if (argv.stages.includes('linux-user')) {
                    ...
                    } else if (!instance.process.createGhostUser()) {
                        // Skip creation if process manager doesn't need it
                        this.ui.log('Selected process manager doesn\'t require the ghost user, skipping', 'yellow');
                    } else {
        ...
        const initialStages = [{
            ...
            title: 'Setting up "ghost" system user',
            task: linux.bind(this),
            enable: ({instance}) => argv['setup-linux-user'] !== false && os.platform() === 'linux' && instance.process.createGhostUser()
        }];
