...
class InstallCommand extends Command {
        ...
        try {
            await this.ui.listr([{
                ...
                task: (ctx, task) => {
                    ...
                    return installDependencies(ctx.ui, ctx.argv.zip);
                }
            }, {
            ...
            // A failed cleanup must not mask the install failure that triggered it
            try {
                await this.cleanInstallDirectory();
            } catch (cleanupError) {
                this.ui.log(`Failed to clean up the install directory: ${cleanupError.message}`, 'yellow');
            }
    ...
    async version(ctx) {
        ...
        ctx.installPath = path.join(process.cwd(), 'versions', resolvedVersion);
    }
    ...
    async cleanInstallDirectory() {
        const cwd = process.cwd();
        await Promise.all(fs.readdirSync(cwd).map(file => fsp.rm(file, {recursive: true, force: true})));
    }
}
