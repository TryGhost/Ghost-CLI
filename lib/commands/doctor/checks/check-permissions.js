...
module.exports = function checkPermissions(type = 'owner', task = 'Checking permissions') {
    let errMsg;
    ...
    return execa(checkTypes[type].command, {maxBuffer: Infinity, shell: true}).then((result) => {
        if (!result.stdout) {
