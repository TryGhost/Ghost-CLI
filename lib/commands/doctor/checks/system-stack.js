...
const deprecatedVersionRegex = /^(?:16|18|20)/;
...
async function checkSystem(ctx) {
    if (!ctx.system.platform.linux) {
        throw new CliError('Operating system is not Linux');
    }

    const {distro, release} = await sysinfo.osInfo();
    if (distro !== 'Ubuntu' || !versionRegex.test(release)) {
        throw new CliError('Linux version is not Ubuntu 16, 18, 20, 22, or 24');
    }

    if (deprecatedVersionRegex.test(release)) {
        ctx.ui.log('Warning: Your Ubuntu version has reached end-of-life.', 'yellow');
        ctx.ui.log('Consider upgrading to Ubuntu 22.04 or 24.04.', 'yellow');
    }
    ...
    if (missing.length) {
        throw new CliError(`Missing package(s): ${missing.join(', ')}`);
    }
}
