...
class DoctorCommand extends Command {
    async run(argv, ctx) {
        ...
        const combinedChecks = checks.concat(extensionChecks.flat().filter(Boolean));

        const checksToRun = argv.categories && argv.categories.length ?
            combinedChecks.filter(check => (check.category || []).some(category => argv.categories.includes(category))) :
            combinedChecks;
