...
class LogCommand extends Command {
    run(argv) {
        let instance = this.system.globalConfig.get(`instances.${argv.name}`);
        ...
        this.system.loadDir(process.cwd());

        // Check if logging file transport is set in config
        if (!includes(this.system.instanceConfig.get('logging.transports', []), 'file')) {
            // TODO: fallback to process manager log retrieval?
            return Promise.reject(new errors.ConfigError({
                configKey: 'logging.transports',
                configValue: this.system.instanceConfig.get('logging.transports').join(', '),
                message: 'You have excluded file logging in your ghost config.' +
                    'Please add it to your transport config to use this command.',
                environment: this.system.mode
            }));
        }

        let logFileName = path.join(process.cwd(), 'content/logs', `${this.system.instanceConfig.get('url').replace(/[^\w]/gi, '_')}_${this.system.mode}.log`);
