            ...
            return this.runCommand(DoctorCommand, Object.assign({
                ...
                return this.ui.run(start, 'Starting Ghost', runOptions).then(() => {
                    ...
                    if (!argv.quiet) {
                        ...
                        if (isInstall && instance.config.get('process') !== 'local' && !urlUtils.isCustomDomain(instance.config.get('url'))) {
                            // Special message for when a production install is completed with an ip or localhost instead of a domain
                            this.ui.log('\n------------------------------------------------------------------------------', 'white');
                            this.ui.log('To add a custom domain and secure your site, run', 'ghost setup ssl --url example.com', 'white', 'cmd');
                        }
