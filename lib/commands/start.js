            ...
            return this.ui.run(start, 'Starting Ghost', runOptions).then(() => {
                ...
                if (!argv.quiet) {
                    ...
                    if (instance.config.get('mail.transport') === 'Direct') {
                        ...
                        const docsURL = chalk.blue.underline('https://docs.ghost.org/docs/mail-config');
                        this.ui.log('\nGhost uses direct mail by default', 'green');
                        this.ui.log(`To set up an alternative email method read our docs at ${docsURL}`, 'green');
                    }
                }
            });
        });
