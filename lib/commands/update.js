...
class UpdateCommand extends Command {
    ...
    async downloadAndUpdate(ctx, task) {
        ...
        if (fs.existsSync(ctx.installPath)) {
            ...
            await fsp.rm(ctx.installPath, {recursive: true, force: true});
        }

        task.title = `Downloading and updating Ghost to v${ctx.version}`;
        return installDependencies(ctx.ui, ctx.zip);
    }

    async removeOldVersions({instance}, task) {
        const semver = require('semver');

        const versionDirs = await fsp.readdir(path.join(instance.dir, 'versions'));
        ...
        const promises = versions.slice(0, -2)
            .map(version => fsp.rm(path.join(instance.dir, 'versions', version), {recursive: true, force: true}));
    ...
    async version(context) {
        ...
        context.version = resolvedVersion;
        context.installPath = path.join(process.cwd(), 'versions', resolvedVersion);
        return true;
    }
    ...
    link({instance, installPath, version, rollback}) {
        fs.rmSync(path.join(process.cwd(), 'current'), {recursive: true, force: true});
