...
// Matches lodash's `isEmpty`: anything that isn't a string, array or object
// (numbers, booleans, etc.) is considered empty
...
function formatTime(date) {
    ...
    const hms = [date.getHours(), date.getMinutes(), date.getSeconds()].map(pad).join(':');
    return `${ymd} ${hms}`;
}
...
const omittedFields = ['time', 'level', 'name', 'hostname', 'pid', 'v', 'msg'];
const omitFields = data => Object.entries(data).filter(([key]) => !omittedFields.includes(key));
...
class PrettyStream extends Transform {
    ...
    _transform(data, enc, cb) {
        ...
        if (data.msg && !data.err) {
            ...
            omitFields(data).forEach(([key, value]) => {
                ...
                if (isObject(value) && value.message && value.stack) {
                    ...
                    if (value.errorDetails) {
                        const errorDetails = Array.isArray(value.errorDetails) ? value.errorDetails[0] : value.errorDetails;
