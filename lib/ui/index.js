...
class UI {
    ...
    constructor(options) {
        ...
        // In json mode stdout is reserved for the machine-readable payload,
        // so prompting (and any other stdout output) is off the table
        this.allowPrompt = this.options.allowPrompt && !this.json && Boolean(this.stdout.isTTY);
    ...
    run(promiseOrFunction, name, options = {}) {
        ...
        // Spinners are also skipped in json mode, since they can't be redirected to stderr.
        if (quiet || this.json) {
            return fn();
        }
    ...
    listr(tasks, context, options) {
        const listrOpts = Object.assign({
            ...
            collectErrors: true,
            // The CLI binds its own SIGINT handler, so don't let Listr add one
            registerSignalListeners: false,
            exitOnError: true
        }, options || {});

        const listr = new Listr(tasks, listrOpts);
        const run = listr.run.bind(listr);

        // When `exitOnError` is false, Listr collects task errors rather than throwing
        // them, so gather up anything collected during this run and throw it instead
        listr.run = async (ctx) => {
            const offset = listr.errors.length;
            const result = await run(ctx);
            const collected = listr.errors.slice(offset).map(({error}) => error);

            if (collected.length) {
                throw new AggregateError(collected, 'Something went wrong');
            }
     ...
     *
     * @param {string[]} command Command to run, as an array of arguments. It's deliberately
     *   not a string: a string would have to be interpreted by a shell, which would let any
     *   value interpolated into it break out of the command it was meant to be part of.
    ...
    sudo(command, options = {}) {
        if (!Array.isArray(command)) {
            throw new errors.CliError('ui.sudo expects an array of arguments');
        }

        // A shell would interpret metacharacters in the arguments, which is exactly what
        // passing them as an array avoids, so callers don't get to turn one back on
        if (options.shell) {
            throw new errors.CliError('ui.sudo cannot run in a shell');
        }

        const prompt = '#node-sudo-passwd#';
        const {sudoArgs = []} = options;
        // A command starting with `ghost` re-runs this CLI, so it has to be the node and
        // ghost binary paths we're currently running rather than whatever `ghost` resolves to
        const args = command[0] === 'ghost' ? [...process.argv.slice(0, 2), ...command.slice(1)] : command;

        this.log(`+ sudo ${args.join(' ')}`, 'gray');

        // No shell, so execa passes each argument to sudo as-is and nothing in them
        // can be interpreted as shell syntax
        const cp = execa('sudo', ['-S', '-p', prompt, ...sudoArgs, ...args], omit(options, ['sudoArgs']));
        ...
        cp.stderr.on('data', (data) => {
            ...
            // The original sudo replaces the whole prompt with the `-p` value, whereas
            // sudo-rs (the default on Ubuntu 26+) wraps it as `[sudo: <prompt>] Password: `,
            // so we can't check for an exact match here
            if (lines[lines.length - 1].includes(prompt)) {
                this.prompt({
                    type: 'password',
                    name: 'password',
        ...
        // Listr treats anything with a `pipe` method as a stream, and execa's subprocess
        // promise has one. Unwrap it so tasks returning `ui.sudo()` aren't mistaken for streams
        return cp.then(result => result);
    }
    ...
    log(message, color, stderr) {
        ...
        // In json mode everything human-readable goes to stderr
        const stream = stderr || this.json ? 'stderr' : 'stdout';
    ...
    /**
     * Writes a machine-readable JSON payload to stdout. Only used by commands
     * run with `--json`
     *
    ...
    error(error, system) {
        ...
        if (error instanceof errors.CliError) {
            ...
            // Listr task failures get aggregated into a single error
