...
class UI {
    ...
    sudo(command, options = {}) {
        ...
        const opts = omit({...options, shell: true}, ['sudoArgs']);

        const cp = execa(`sudo -S -p '${prompt}' ${options.sudoArgs || ''} ${cmd}`, opts);
    ...
    error(error, system) {
        ...
        if (error instanceof errors.CliError) {
            ...
            this.log(`\nTry running ${(chalk.cyan('ghost doctor'))} to check your system for known issues.`);
            this.log('\nYou can always refer to https://ghost.org/docs/api/v2/ghost-cli/ for troubleshooting.', 'blue');
