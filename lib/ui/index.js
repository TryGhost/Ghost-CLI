...
function hasDefault(prompt = {}) {
    return isObject(prompt) && prompt.default !== null && prompt.default !== undefined;
}
...
class UI {
    ...
    run(promiseOrFunction, name, options = {}) {
        ...
        const fn = () => Promise.resolve(typeof promiseOrFunction === 'function' ? promiseOrFunction() : promiseOrFunction);
    ...
    sudo(command, options = {}) {
        ...
        const {sudoArgs, ...execaOptions} = options;
        const cp = execa(`sudo -S -p '${prompt}' ${sudoArgs || ''} ${cmd}`, {
            ...execaOptions,
            shell: true
        });
    ...
    noSpin(promiseOrFunc) {
        ...
        return Promise.resolve(typeof promiseOrFunc === 'function' ? promiseOrFunc() : promiseOrFunc).then((result) => {
            if (this.spinner) {
                this.spinner.start();
