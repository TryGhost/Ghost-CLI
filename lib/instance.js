...
class Instance {
     ...
     * @param {import('./system')} system System instance
    ...
    async isRunning() {
        if (!this._cliConfig.has('running')) {
            const discoverEnvironments = require('./utils/discover-envs');
            const currentEnvironment = this.system.environment;
            const knownEnvironments = discoverEnvironments(this.dir);

            /** @param {string} environment */
            const envIsRunning = async (environment) => {
                this.system.setEnvironment(environment);
            ...
            for (const environment of knownEnvironments) {
                if (await envIsRunning(environment)) {
                    return true;
                }
            }
     ...
     */
    checkEnvironment() {
        const {environment} = this.system;
        const currentEnvironmentConfigExists = Config.exists(path.join(this.dir, `config.${environment}.json`));

        if (currentEnvironmentConfigExists) {
            return;
        }

        if (environment === 'production' && Config.exists(path.join(this.dir, 'config.development.json'))) {
            this.ui.log(
        ...
        this.ui.log(
            `The ${environment} config (config.${environment}.json) does not exist; can't continue`,
            'red'
        );

        throw new CliError({
            message: 'Missing environment configuration file'
        });
    }
     ...
     *
     * @param {boolean} setNodeEnv should the NODE_ENV environment variable also be set?
     *
    ...
    loadRunningEnvironment(setNodeEnv) {
        ...
        this.system.setEnvironment(env, setNodeEnv);
    }
