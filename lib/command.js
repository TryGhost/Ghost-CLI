...
class Command {
    ...
    static async _run(commandName, argv = {}, extensions) {
        ...
        /**
         * Detect what environment to run
         * From user input (overrides operating system default):
         * 1. If the user passes --development or -d, use development
         * 2. If the user passes --environment, use whatever they provided
         * 3. Pull from NODE_ENV
         *
         * From Operating System:
         * 1. Linux is the only officially supported production environment, so default to production
         * 2. All other Operating Systems are assumed to be development
         */
        const explicitEnvironment = (argv.development && 'development') || argv.environment || process.env.NODE_ENV;
        const fallbackEnvironment = system.platform.linux ? 'production' : 'development';
        // @todo(node14+) - use nullish coalescing
        // const environment = explicitEnvironment ?? fallbackEnvironment
        const environment = explicitEnvironment === undefined ? fallbackEnvironment : explicitEnvironment;
        system.setEnvironment(environment, true);
