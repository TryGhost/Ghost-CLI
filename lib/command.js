...
class Command {
    ...
    static configure(commandName, aliases, yargs, extensions) {
        if (!this.description) {
            throw new CliError(`Command ${commandName} must have a description!`);
        }
     ...
     */
    async run() {
        throw new CliError('Command must implement run function');
    }
    ...
    async runCommand(CommandClass, argv) {
        if (!(CommandClass.prototype instanceof Command)) {
            throw new CliError('Provided command class does not extend the Command class');
        }
